...
async function test() {
    ...
    const swiggyRule = rules.find(r => r.keyword === 'swiggy');
    console.log('Swiggy rule found:', !!swiggyRule);
    
    const { data: expenses } = await sb.from('expenses_row').select('*');
    
    const batch5Keys = ['swiggy', 'refund', '1stvideoj60', '2ndvideoj61'];
    
    let count = 0;
    expenses.forEach(tx => {
        const desc = normalizePdfSpacing(tx.notes || '');
        const matched = rules.filter(r => matchesKeyword(desc, r.keyword));
        const batch5Matched = matched.filter(r => batch5Keys.includes(r.keyword.toLowerCase()));
        
        if (batch5Matched.length > 0) {
            console.log(tx.notes, ' => ', batch5Matched.map(r=>r.keyword));
            count++;
        }
    });
    console.log('Total batch 5 matches:', count);
}
test();
