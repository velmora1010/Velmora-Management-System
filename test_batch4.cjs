...
async function test() {
    ...
    const batch4Keys = ['dovefragrance24', 'ncap10ltr'];
    
    let count = 0;
    expenses.forEach(tx => {
        const desc = normalizePdfSpacing(tx.notes || '');
        const matched = rules.filter(r => matchesKeyword(desc, r.keyword));
        const batch4Matched = matched.filter(r => batch4Keys.includes(r.keyword.toLowerCase()));
        
        if (batch4Matched.length > 0) {
            console.log(tx.notes, ' => ', batch4Matched.map(r=>r.keyword));
            count++;
        }
    });
    console.log('Total batch 4 matches:', count);
}
test();
