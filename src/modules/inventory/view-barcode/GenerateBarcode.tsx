...
const GenerateBarcode = () => {
  ...
  const downloadQR = (batchId: string) => {
    const el = document.getElementById(`qr-${batchId}`);
    if (!el) return;
    const svg = el.querySelector('svg');
    if (svg) {
      barcodeService.downloadSVG(svg as SVGSVGElement, `Barcode-${batchId}.svg`);
    } else {
      barcodeService.downloadPNG(el, `Barcode-${batchId}.png`);
    }
  };

  const printQR = (batchId: string) => {
    const scanCode = barcodeService.deriveScanCode(batchId, 'RAW_MATERIAL');
