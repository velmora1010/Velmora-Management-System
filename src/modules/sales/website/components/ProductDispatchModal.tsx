      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden space-y-4">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-extrabold text-white tracking-tight">Products Dispatched Breakdown</h3>
              <p className="text-xs text-slate-400">Total {totalUnits} units dispatched</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 pt-0 space-y-3 max-h-[60vh] overflow-y-auto">
          {products.length === 0 ? (
            <div className="p-6 text-center text-xs text-slate-500">
              No products dispatched for the selected date/filters.
            </div>
          ) : (
              ...
              <table className="w-full text-left text-xs text-slate-300">
                <thead className="bg-slate-950 text-[10px] uppercase font-bold text-slate-400 border-b border-slate-800">
                  <tr>
                    ...
                    <th className="py-2.5 px-4 text-right">Units Dispatched</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/60">
                  {products.map(p => (
                    <tr key={p.product} className="hover:bg-slate-800/40 transition-colors">
                      <td className="py-2.5 px-4 font-bold text-white truncate max-w-[260px]">{p.product}</td>
                      <td className="py-2.5 px-4 text-right font-mono font-bold text-emerald-400">{p.units}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
