     ...
     <div className="bg-slate-900 border border-slate-700 p-4 rounded-xl hover:border-slate-600 transition-colors relative mb-6">
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 mb-4 pb-3 border-b border-slate-800/60 min-w-0">
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-auto">
            {onDispatch && (
              isInfluencerReDispatch(influencer) ? (
                <div className="flex items-center gap-1.5 shrink-0">
                  <span className="px-2 py-0.5 text-[10px] font-bold rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 uppercase tracking-wider">
                    Re-Dispatch
                  </span>
                  <button 
                    type="button"
                    onClick={() => onDispatch(influencer)} 
                    className="px-3 py-1 text-xs font-bold rounded-md bg-amber-600 hover:bg-amber-500 text-slate-950 transition-colors shrink-0 shadow-sm cursor-pointer shadow-amber-500/20"
                  >
                    Re-Dispatch
                  </button>
                </div>
                    ...
                    <div key={vNum} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50">
                      ...
                      {videoProds.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {videoProds.map((p, pIdx) => (
                            <div key={pIdx} className="flex justify-between items-center bg-slate-900 p-2.5 rounded border border-slate-700">
                              <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${p.selected !== false ? 'bg-emerald-400' : 'bg-slate-600'}`}></div>
                                <span className={`text-xs ${p.selected !== false ? 'text-slate-200 font-medium' : 'text-slate-500 line-through'}`}>{p.name}</span>
                              </div>
