...
export class IciciCreditPdfParser implements DocumentParser {
  async parse(rawContent: PdfTextItem[], file: File): Promise<NormalizedTransaction[]> {
      ...
      for (const row of dataRows) {
        ...
        for (const item of row) {
          ...
          if (colName && colName !== 'sequence') {
            (logicalRow as any)[colName] += (logicalRow[colName] ? ' ' : '') + item.text;
          }
        }
    ...
    for (const row of mergedRows) {
      ...
      const depositAmt = cleanMoney(row.deposit);
      if (depositAmt <= 0) {
        continue;
      }
    ...
    if (validTransactions.length === 0) {
      throw new Error(
        `Could not find any valid credit/deposit transactions in ${file.name}. ` +
        `Reconstructed ${mergedRows.length} logical rows but none passed validation. ` +
        `Please ensure this statement contains credit transactions.`
      );
    }
