...
export default function OtherStatePage() {
  const shipments = useLiveQuery(() => 
    db.shipments.filter(s => s.department === 'Other State').toArray()
  ) || [];
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <div>
          <h1>Other State Department</h1>
          <p>Showing all shipments routed to states outside Tamil Nadu.</p>
        </div>
              ...
              {shipments.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{textAlign: 'center', padding: '2rem'}}>
                    No shipments found in Other State department.
                  </td>
                </tr>
              ) : shipments.map((item) => (
                <tr key={item.awb}>
                  ...
                  <td>{item.lastLocation} ({item.state})</td>
