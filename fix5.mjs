...
const targetIndex = content.indexOf(`{/* BOTTOM ROW: Value & Batches */}`);

if (targetIndex > -1) {
  // Let's replace the corrupted region with the correct one
  const regex = /\{\/\* BOTTOM ROW: Value & Batches \*\/\}\s*<div style=\{\{ display: 'flex', justifyContent: 'space-between', padding: '0 4px' \}\}>\s*<div style=\{\{ display: 'flex', flexDirection: 'column', gap: '4px' \}\}>\s*<span style=\{\{ fontSize: '10px', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase' \}\}>Value<\/span>\s*<span style=\{\{ fontSize: '14px', fontWeight: 700, color: '#e2e8f0' \}\}>₹\{card\.amount\.toLocaleString\(undefined, \{ maximumFractionDigits: 0 \}\)\}<\/span>\s*<th style=\{\{ padding: '16px 20px', fontWeight: 600, color: '#94a3b8', borderBottom: '1px solid #334155', textTransform: 'uppercase', fontSize: '12px', letterSpacing: '0\.5px' \}\}>Status<\/th>\s*<th style=\{\{ padding: '16px 20px', fontWeight: 600, color: '#94a3b8', borderBottom: '1px solid #334155', textTransform: 'uppercase', fontSize: '12px', letterSpacing: '0\.5px' \}\}>Action<\/th>\s*<\/tr>\s*<\/thead>/;

  const correctRegion = `{/* BOTTOM ROW: Value & Batches */}
  ...
  content = content.replace(regex, correctRegion);
