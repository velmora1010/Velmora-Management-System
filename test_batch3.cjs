...
async function test() {
    ...
    const batch3Keys = ['zguard', 'vinayagapl', 'whiteflower10bo'];
    
    let count = 0;
    expenses.forEach(tx => {
        const desc = normalizePdfSpacing(tx.notes || '');
        const matched = rules.filter(r => matchesKeyword(desc, r.keyword));
        const batch3Matched = matched.filter(r => batch3Keys.includes(r.keyword.toLowerCase()));
        
        if (batch3Matched.length > 0) {
            console.log(tx.notes, ' => ', batch3Matched.map(r=>r.keyword));
            count++;
        }
    });
    console.log('Total batch 3 matches:', count);
}
test();
